...
export const formatTimestamp = (timestamp: string) => {
  ...
  return diff > hours24 ? date.toLocaleDateString() : date.toLocaleTimeString();
};
...
export const formatValue = (val: number, format: string): string => {
  switch (format) {
    case "bytes":
...
const fetchPrometheusData = async (
  ...
  const metricsApiUrl = getMetricsApiUrl();
  const responses = await Promise.all(
    queries.map((query) =>
      fetch(
        `${metricsApiUrl}/api/v1/query_range?query=${encodeURIComponent(
          query,
  ...
  try {
    ...
    const uptimeResponses = await Promise.all(
      Object.values(uptimeQueries).map((query) =>
        fetch(
          `${metricsApiUrl}/api/v1/query?query=${encodeURIComponent(
            query,
          )}`,
        ).then((res) => res.json()),
      ),
...
export const fetchNodeDetails = async (instance: string) => {
  try {
    const queries = {
      cpuModel: `node_cpu_info{instance="${instance}"}`,
    ...
    responses.forEach(({ key, data }) => {
      if (data?.data?.result?.[0]) {
        if (key === "cpuCores") {
          ...
          details.cpuModel = data.data.result[0].metric.model_name;
        }
      }
    });
...
export const fetchNodeName = async (instance: string) => {
  ...
  const response = await fetch(
    `${metricsApiUrl}/api/v1/query?query=node_uname_info{instance="${instance}"}`,
  );
...
// Get the metrics API URL from environment variables
const getMetricsApiUrl = (): string => {
  const apiUrl = process.env.NEXT_PUBLIC_METRICS_API_URL;
  if (!apiUrl) {
    throw new Error(
      "NEXT_PUBLIC_METRICS_API_URL environment variable is not set. Please configure it in your .env file.",
    );
  }
  return apiUrl;
};
