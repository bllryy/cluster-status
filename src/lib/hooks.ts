          ...
          data.data.activeTargets
            ...
            .map(async (target: TargetResponse): Promise<Node> => {
              ...
              const [nodenameRes, uptimeRes] = await Promise.all([
                fetch(
                  `${metricsApiUrl}/api/v1/query?query=node_uname_info{instance="${target.labels.instance}"}`,
                ),
                fetch(
                  `${metricsApiUrl}/api/v1/query?query=(node_time_seconds{instance="${target.labels.instance}"} - node_boot_time_seconds{instance="${target.labels.instance}"})`,
                ),
              ]);
